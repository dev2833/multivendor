...
const TenPlusOne = mongoose.model('TenPlusOne');
exports.create = (req, res) => {
    const tenPlusOne = new TenPlusOne(req.body);
    tenPlusOne.save((err) => {
        if (err) {
            res.status(404).json(err);
        } else {
            console.log(TenPlusOne)
            res.status(200).json({tenPlusOne:tenPlusOne});
        }
    });
};
...
exports.list = (req, res) => {
    ...
    if( role === "Vendor" ){
        TenPlusOne.find(function(err, tenPlusOnes) {
            if (err) {
                res.json(err);
            } else {
                res.json(tenPlusOnes);
            }
...
exports.listByCustomer = (req, res) => {
    ...
    TenPlusOne.find({customerId:req.params.id},function(err, tenPlusOnes) {
            if (err) {
                res.json(err);
            } else {
                res.json(tenPlusOnes);
